...
<div class="max-w-6xl mx-auto" x-data="{ editId: {{ old('_edit_id', 'null') }} }">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
        ...
        <form action="{{ route('admin.teams.section.update') }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block ml-1">Titre</label>
                    <input type="text" name="teams_title" value="{{ old('teams_title', $section['title']) }}" class="input-field" placeholder="Nos équipes en action">
                </div>
                <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block ml-1">Sous-titre</label>
                    <textarea name="teams_subtitle" rows="3" class="input-field" placeholder="Découvrez notre expertise...">{{ old('teams_subtitle', $section['subtitle']) }}</textarea>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div class="flex items-center bg-slate-100 px-4 py-3 mb-6">
            <h2 class="text-xs font-semibold text-slate-600 leading-tight uppercase tracking-widest" style="font-family: 'Outfit', sans-serif;">IMAGES DE LA SECTION</h2>
        </div>
        ...
        <div class="flex justify-end mb-4" x-data="{ showCreate: {{ $errors->any() ? 'true' : 'false' }} }">
            ...
            <!-- Fenêtre modale : ajout d'une image -->
            <div x-show="showCreate" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4" style="display: none;">
                ...
                <div class="relative bg-white rounded-xl shadow-2xl border border-slate-200 w-full text-left" style="max-width: 42rem; max-height: 90vh; overflow-y: auto;">
                    <div class="flex items-center justify-between bg-slate-100 px-4 py-3 sticky top-0 z-10">
                        <h2 class="text-xs font-semibold text-slate-600 leading-tight uppercase tracking-widest" style="font-family: 'Outfit', sans-serif;">AJOUTER UNE IMAGE</h2>
                    ...
                    <form action="{{ route('admin.teams.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="text" name="caption" value="{{ old('caption') }}" class="input-field">
                            </div>
                        </div>
            ...
            <table class="datatable">
                <thead>
                    <tr>
                        ...
                        <th class="!text-slate-900">Légende</th>
                ...
                <tbody>
                    ...
                    <tr class="group">
                        <td>
                            @if($item->imageUrl())
                                <img src="{{ $item->imageUrl() }}" alt="{{ $item->caption }}" class="w-20 h-14 object-cover rounded-lg border border-slate-200">
                            @else
                                <div class="w-20 h-14 rounded-lg bg-slate-100 border border-slate-200"></div>
                            @endif
                        </td>
                        <td><span class="text-sm font-bold text-slate-900">{{ $item->caption }}</span></td>
                        <td class="text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.teams.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer cette image ?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="3" class="px-8 py-16 text-center">
                            <p class="text-sm font-semibold text-slate-400">Aucune image pour le moment</p>
                            <p class="text-xs text-slate-300 mt-1">Cliquez sur « Ajouter une image » pour commencer.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        {{-- Modales d'édition (une par image) --}}
        @foreach($items as $item)
            <div x-show="editId === {{ $item->id }}" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center p-4" style="display:none;">
                ...
                <div class="relative bg-white rounded-xl shadow-2xl border border-slate-200 w-full text-left" style="max-width: 42rem; max-height: 90vh; overflow-y: auto;">
                    <div class="flex items-center justify-between bg-slate-100 px-4 py-3 sticky top-0 z-10">
                        <h2 class="text-xs font-semibold text-slate-600 uppercase tracking-widest" style="font-family: 'Outfit', sans-serif;">MODIFIER L'IMAGE</h2>
                    ...
                    <form action="{{ route('admin.teams.update', $item) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="p-6 space-y-6">
                            <div class="space-y-2">
                                ...
                                @if($item->imageUrl())
                                    <img src="{{ $item->imageUrl() }}" alt="{{ $item->caption }}" class="w-full max-w-sm rounded-lg border border-slate-200 object-cover aspect-[4/3]">
                                @endif
                            </div>
                            <div class="space-y-2">
                            ...
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-500 uppercase tracking-widest block ml-1">Légende</label>
                                <input type="text" name="caption" value="{{ old('caption', $item->caption) }}" class="input-field">
                            </div>
                        </div>
                        <div class="p-6 border-t border-slate-100 flex justify-end gap-3">
                            <button type="submit" class="btn-primary" style="border-radius: 3px; padding: 0.625rem 3rem; font-size: 0.8125rem">Enregistrer</button>
