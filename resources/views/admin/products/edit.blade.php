...
<div class="max-w-4xl mx-auto space-y-10 pb-20">
    ...
    <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="premium-card p-10 space-y-10">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-4 gap-4 mb-8">
                    @if(!empty($product->images))
                        @foreach($product->images as $img)
                            <div class="relative aspect-square rounded-xl overflow-hidden border border-slate-100 shadow-sm">
